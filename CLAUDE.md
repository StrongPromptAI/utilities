...
This repo is a **collection of utilities** — shared services (embed/STT/TTS/whisper), the hj-roadmap site + sales-coach chatbot, the kb stakeholder CLI, radar, oxp-kb/oxp-files, and internal tooling — that share a tree, a Railway Postgres, and a skill set. They are independently deployed; don't assume one app.

---

## Registries & Project Docs

Catalogs of this repo's pieces. Know they exist; read the relevant one on demand rather than carrying every detail here. They exist so facts stop being re-derived from the tree each session.
...
How to authenticate into every utilities surface — hj-roadmap (roadmap + DME sales-coach chat) human OTP login, the user-JWT mint for API / streaming-chat testing, the admin debug/ingest token, the shared-svcs service tokens (embed/STT/TTS/whisper), and oxp-kb SSO — lives in one place:

**[`symlink_docs/registries/AUTH_REGISTRY.md`](symlink_docs/registries/AUTH_REGISTRY.md)** — recipes + secrets-location per surface, scenario-organized.

That registry holds the *recipes*; the operational *values* it delegates to live below in this file (§ Railway for IDs/var-pulls, § Shared Services for the token-mint contract).

---

## Railway

> App-surface auth (OTP login, user-JWT mint, admin token) → [`symlink_docs/registries/AUTH_REGISTRY.md`](symlink_docs/registries/AUTH_REGISTRY.md). Below is infra/API auth only.
...
Self-hosted OpenWebUI stack on Railway project `oxp-kb`. Public surface: `oxp.chat`.
